...
export default function NotificationPrefsToggle() {
  ...
  return (
    <div>
      <h3 className="text-sm font-semibold text-slate-700 mb-4">Email notifications</h3>
      <div className="flex items-center justify-between py-3 px-4 bg-slate-50 rounded-lg">
        <div className="flex items-center gap-3">
          {enabled ? <Bell size={18} className="text-indigo-500" /> : <BellOff size={18} className="text-slate-400" />}
          <div>
            <p className="text-sm font-medium text-slate-700">{enabled ? 'Notifications enabled' : 'Notifications disabled'}</p>
            <p className="text-xs text-slate-400">Affects all users — assigned and status change emails</p>
          </div>
        </div>
        <button
          onClick={toggle}
          disabled={saving}
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${enabled ? 'bg-indigo-600' : 'bg-slate-300'} ${saving ? 'opacity-50' : ''}`}
        >
