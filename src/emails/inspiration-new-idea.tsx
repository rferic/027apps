...
const labels = (locale: string) => {
  const all: Record<string, { preview: string; heading: string; authorLine: string; cta: string; footer: string; noDesc: string }> = {
    en: {
      preview: 'New idea submitted — 027Apps',
      heading: 'New idea submitted',
      authorLine: '{author} proposed:',
      cta: 'View idea',
...
export default function InspirationNewIdeaEmail({ authorName, title, description, requestUrl, locale }: Props) {
          ...
          <table cellPadding="0" cellSpacing="0" style={tableInner}>
            ...
            <tr>
              <td style={contentSection}>
                ...
                <p style={paragraph}>
                  {l.authorLine.replace('{author}', `<strong>${authorName}</strong>`)}
                </p>
